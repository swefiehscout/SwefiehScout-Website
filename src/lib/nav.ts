...
// top-level slug. The "Home" page is rendered separately via the site logo, so
// it isn't included in the returned list.
export async function getNav(): Promise<NavItem[]> {
  const pages = await getCollection('pages');

  const topLevel = pages
    .filter((p) => p.data.navOrder !== undefined && p.data.slug !== 'index')
